...
def get_geckodriver():
    gecko_path = shutil.which("chromedriver") or shutil.which("chromedriver.exe")
    if gecko_path:
        return gecko_path
    executable_path=ChromeDriverManager().install()
    sym_path = executable_path
    return sym_path
...
def set_selenium_local_session(
    ...
    firefox_options.add_argument("window-size=1920x1480")
    ...
    if browser_profile_path is not None:
        firefox_options.add_argument('--profile-directory=Profile 1')
    else:
        pass

    #if browser_executable_path is not None:
#        firefox_options.binary_location = os.environ.get("GOOGLE_CHROME_BIN")
    if proxy_address and proxy_port:
        pass
    
    if disable_image_load:
        firefox_options.add_extension("Block-image_v1.1.crx")
    ...
    driver_path = geckodriver_path or ChromeDriverManager().install()
    browser = webdriver.Chrome(
        executable_path=ChromeDriverManager().install(),
        options=firefox_options,
        service_args=["--verbose", "--log-path="+geckodriver_log]
    )
